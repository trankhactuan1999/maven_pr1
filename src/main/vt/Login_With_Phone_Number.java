...
public class Login_With_Phone_Number extends BaseTest {
    ...
    public void Login_01_Open_Login_Popup() {
        ...
        homePage.clickIfElemenIsPresent(driver, BasePaseUI.ClOSE_BUTTON_ON_BANNER, 6);
    ...
    @Test
        ...
        homePage.enterTextboxByName(driver,"number","0342992917");
        ...
        homePage.clickToLoginButtonOnPopup(driver);
        log.info("Step 10: Verify message is displayed");
        verifyEquals(homePage.MessageIncorrectPassword(driver),"Tài khoản hoặc mật khẩu không đúng, Xin Quý khách vui lòng thao tác lại.");
        log.info("Step 14: Click on 'Quay lại' button");
        homePage.clickToBackButton(driver);
    }

    @Test
    public void Login_04_Login_Successfully_With_Password() {
        log.info("Step 15: Enter valid password");
        homePage.enterTextboxByName(driver,"password", "vt123456a@");
        log.info("Step 16: Click on 'Đăng nhập' button");
        homePage.clickToLoginButtonOnPopup(driver);
        homePage.clickIfElemenIsPresent(driver, BasePaseUI.ClOSE_BUTTON_ON_BANNER, 6);
        log.info("Step 17: Verify message 'Xin chao' is displayed");
        verifyTrue(homePage.messageHelloIsDisplayed(driver));
    }
